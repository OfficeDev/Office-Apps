...
const ADD_IN_CACHE = 'add-in-cache-v' + CURRENT_VERSION;

const ADD_IN_CACHE_URLS = [
...
self.addEventListener('install', event => {
    event.waitUntil(
        caches.open(ADD_IN_CACHE)
            .then(cache => cache.addAll(ADD_IN_CACHE_URLS))
...
self.addEventListener('activate', event => {
    event.waitUntil(
        caches.keys().then(cacheNames => {
            return cacheNames.filter(cacheName => ADD_IN_CACHE != cacheName);
        }).then(cachesToDelete => {
    ...
    if (event.request.method != 'POST') {
        event.respondWith(
            caches.match(event.request, {ignoreSearch: true}).then(cachedResponse => {
                ...
                return caches.open(ADD_IN_CACHE).then(cache => {
